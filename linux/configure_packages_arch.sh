...
sudo pacman -S --noconfirm --needed hexyl # Terminal hex viewer
...
sudo pacman -S --noconfirm --needed ripgrep # Fast regex text searching tool for files (recursively), respects .gitignore
...
sudo pacman -S --noconfirm --needed git-delta # Syntax-highlighting pager for git and diff output
